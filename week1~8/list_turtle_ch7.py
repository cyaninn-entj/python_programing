...
if __name__ == "__main__" :
    ...
    re_tur = sorted(PlayerTurtles, key=lambda turtles : turtles[7] ,reverse = True) #구글링 사용
    #print(PlayerTurtles)
    ...
    for index, tList in enumerate(PlayerTurtles[0:]) :
        ...
        if index == 0 :
            myTurtle.goto(tList[1], tList[2])
            continue
        myTurtle.goto(re_tur[index-1][1], re_tur[index-1][2]) # 선을 그을 거북이를 이 전의 거북이 위치로 이동 - googling
        myTurtle.pendown()
        myTurtle.goto(tList[1], tList[2])   #설정된 거북이의 좌표로 이동하면서 선 긋기 - googling
